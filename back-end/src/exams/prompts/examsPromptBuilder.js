...
export function buildGenerateExamPrompt({
  ...
  const systemPrompt = [
    ...
    "Use the slot marks exactly as provided; slot marks are fixed positive integers.",
    "A question has one main grade only (the slot mark).",
    "If question_text contains multiple sub-items (multiple non-empty lines), the main grade must split equally with integer value per sub-item.",
    "Do not introduce decimal marks anywhere.",
  ...
  const payload = {
    ...
    exam_context: {
      ...
      marks_rule:
        "Each slot already carries a fixed positive integer mark. Keep one main grade per question, do not add a marks field, and ensure any multi-line sub-items can split equally by integer.",
    },
