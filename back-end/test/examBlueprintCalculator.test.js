...
test("buildExamBlueprint keeps every question mark as an integer", () => {
  const lessons = [
    { id: 1, name: "الدرس الأول", number_of_periods: 2 },
    { id: 2, name: "الدرس الثاني", number_of_periods: 1 },
  ];
  ...
  assert.ok(perQuestionMarks.every((mark) => mark >= 1));
  assert.ok(perQuestionMarks.every((mark) => Number.isInteger(mark)));
  assert.equal(perQuestionMarks.reduce((sum, mark) => sum + mark, 0), 6);
});
